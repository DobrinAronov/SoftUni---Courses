...
def longer_line(x_1_frs: float, y_1_frs: float, x_2_frs: float, y_2_frs: float,
    ...
    length_first_line = sqrt((x_frs_max - x_frs_min) ** 2 + (y_frs_max - y_frs_min) ** 2)
    length_second_line = sqrt((x_sec_max - x_sec_min) ** 2 + (y_sec_max - y_sec_min) ** 2)
    ...
    if length_first_line >= length_second_line:
        the_longer_line = coordinates_first_line
    else:
        the_longer_line = coordinates_second_line

    closest_point = coordinates_closest_point(the_longer_line)

    if closest_point == the_longer_line[0]:
        closest_point = the_longer_line[0]
        farthest_point = the_longer_line[1]
    else:
        closest_point = the_longer_line[1]
        farthest_point = the_longer_line[0]
